...
knn.fit(X_train, y_train)  # This line has an issue, see error message below
...
# Handling the K-Nearest Neighbors error
if isinstance(y_pred, Exception):
    print("K-Nearest Neighbors prediction error:", y_pred)
else:
    print("K-Nearest Neighbors Accuracy:", round(accuracy_score(y_test, y_pred) * 100, 2), "%")
...
print("Soft Voting Ensemble Accuracy:", round(accuracy_score(y_test, y_pred) * 100, 2), "%")
...
print("Hard Voting Ensemble Accuracy:", round(accuracy_score(y_test, y_pred) * 100, 2), "%")
